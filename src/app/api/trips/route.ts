...
import { generateTripPlan } from '@/lib/api/openai'

// API implementation that tries OpenAI first, then falls back to mock data
export async function POST(req: Request) {
  try {
    ...
    try {
      // Try to use OpenAI for AI-powered trip planning
      const openAITripPlan = await generateTripPlan(validatedData);
      
      // Add the generated trip ID
      openAITripPlan.id = tripId;
      
      return NextResponse.json(openAITripPlan, { status: 201 })
    } catch (openAIError) {
      console.error('OpenAI integration failed, falling back to mock implementation:', openAIError);
      
      // If OpenAI fails, use our mock implementation as fallback
      const mockTripPlan = await generateMockTripPlan(validatedData, tripId);
      return NextResponse.json(mockTripPlan, { status: 201 })
    }
  } catch (error) {
...
// Mock implementation for generating trip plans
async function generateMockTripPlan(validatedData: TripPreferences, tripId: string) {
  ...
  for (let i = 0; i < tripDuration; i++) {
    ...
    dailyPlans.push({
      ...
      meals,
      accommodation: {
        name: `${currentDestination.split(',')[0]} ${
